...
`go-ruby-dry-struct/dry-struct` reimplements Ruby's `dry-struct` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-dry-struct/dry-struct` directly, with no Ruby runtime;
